...
for(i in 1:20)
{
  train.index = sample(1:270,170,replace=FALSE)
  xtrain = heartt[train.index,c(1,4,5,8,10,12)]
  ytrain = heartt[train.index,14]
  xtest = heartt[-train.index,c(1,4,5,8,10,12)]
  ytest = heartt[-train.index,14]

  #Linear SVMs (SVC)
  y=as.factor(ytrain)
  traindat1 = cbind(xtrain,y)
  tune.out.SVC=tune(svm, y~ age+rbp+chol+hrate+oldpeak+majorV, data=traindat1,kernel="linear",
          ranges = list(cost=c(0.001,0.01,0.1,1,5,10,100)))
  bestmod.SVC = tune.out.SVC$best.model
  pred.SVC =predict(bestmod.SVC, testdat)
  
  errTs.SVC[i] = sum(ytest!=pred.SVC)/length(pred.SVC)
  
#Kernel SVMs - radial kernels
  tune.out.svm=tune(svm,y ~ age+rbp+chol+hrate+oldpeak+majorV, data=traindat1,kernel="radial",
              ranges =list(cost=c(0.001,0.005,0.0075,0.01,0.05,0.075,0.1,0.5,0.75,1,1.5,2,5,10,100),gamma = c(0.1,0.5,1,2,3,4)))
  bestmod.svm = tune.out.svm$best.model
  pred.SVM =predict(bestmod.svm, testdat) 
  errTs.SVM[i] = sum(ytest!=pred.SVM)/length(pred.SVM)
  }

errTs = c(errTs.knn, errTs.lda, errTs.qda, errTs.lr, errTs.lasso, errTs.SVC, errTs.SVM)
indicator = c(rep("knn",20),rep("LDA",20),rep("QDA",20),rep("LR",20),
rep("Lasso",20),rep("Linear-SVM",20),rep("radial-SVM",20))
